...
public class RouteManagerService implements RouteManager {
    ...
    /**
     * This bean provides the possiblity to configure camel contexts. Its functions will be called if a new {@link CamelContext} is started.
     *
     * @return a camel context configuration
     */
    @Bean
    ...
    public List<RouteComponent> listComponents() {
        String[] beanNamesForType = ctx.getBeanNamesForType(ComponentConfigurationPropertiesCommon.class);

        // Yes, this is a bit hacky but will do for now
        return Arrays.stream(beanNamesForType).map(name -> {
            String first = name.split("-org")[0];

            if (first.equals("camel.component")) {
                return null;
            }

            return new RouteComponent("camel-" + first.split("camel.component.")[1], "");
    ...
    private List<CamelContext> getCamelContexts() {
        List<CamelContext> camelContexts = new ArrayList<>(ctx.getBeansOfType(CamelContext.class).values());
