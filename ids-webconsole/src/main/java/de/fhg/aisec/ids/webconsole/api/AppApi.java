...
public class AppApi {
  private static final long PULL_TIMEOUT_MINUTES = 20;
  private static final Logger LOG = LoggerFactory.getLogger(AppApi.class);

  private @NonNull ContainerManager cml;
  private @NonNull Settings settings;

  public AppApi(@Autowired @NonNull ContainerManager cml, @Autowired @NonNull Settings settings) {
    this.cml = cml;
  ...
  public String install(
    ...
    final Future<String> handler =
        executor.submit(
            () -> {
              Optional<String> containerId = cml.pullImage(app);
